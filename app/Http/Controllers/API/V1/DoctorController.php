...
class DoctorController extends Controller
{
    ...
    {
        try {
            // Start the query by filtering by department ID and hidden = 0
            $query = Consultant::where('DepartmentID', $departmentId)
                ->where('hidden', 0) // Include only records where hidden = 0
                ->with('consultantShift');
            ...
            $doctorData = $doctors->unique('ConsultantName')->map(function ($doctor) {
                return [
                    ...
                    'Fee' => optional($doctor->consultantShift)->Fee
                        ? number_format(optional($doctor->consultantShift)->Fee, 2)
                        : null,
                ];
            });
    ...
    public function getAllConsultants()
    {
        ...
        $consultants = Consultant::where('hidden', 0) // Exclude hidden consultants
            ->with(['department', 'consultantShift'])
            ->get();
